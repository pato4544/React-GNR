...
const Contacto: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const res = await fetch ("https://serverless-sigma-jade.vercel.app/api/contact-data",
      {
        method: 'POST',
        headers: {
    ...
    if (res.ok) {
      ...
      console.error("Fallo al enviar los datos del formulario", await res.text());
    }
  ...
  useEffect(() => {  // Si cumple las condiciones, el boton se habilita, si no sigue deshabilitado (las dependencias son los valores de los inputs obligatorios)
    if (nombre !== "" && apellido !== "" && emailValido(email) && mensaje !== "" && selectedValue !== null) {
      setDisabled(false);
    } else {
      setDisabled(true);
    }
  }, [selectedValue, nombre, apellido, email, mensaje]);
      ...
      <div className='min-h-screen flex justify-center h-auto w-full bg-black pt-20 pb-10'>
        <form onSubmit={handleSubmit} className='flex flex-col items-center text-white gap-12 w-[50%] place-self-center '>
          <div className='w-[80%]'>
            ...
            <Select selectedValue={selectedValue} onChange={setSelectedValue} options={options} />  {/* Le pasamos el array de arriba como parametro */}
          </div>
          ...
          <button disabled={disabled} onClick={() => navigate("/contacto/enviado")} onPointerEnter={() => {if (disabled) {setMsjbutton("Campos obligatorios faltantes")}}} onPointerLeave={() => {if (disabled) {setMsjbutton("Enviar")}}} className={` ${disabled ? "hover:bg-[#3b3b3b]" : "hover:bg-yellow-500"} pointer-events-auto  border-2 h-8 disabled:border-gray-400 disabled:text-gray-400 text-white font-bold transition duration-300 rounded-full w-[75%]`} ><p>{msjbutton}</p></button>
