...
const Album = () => {
  ...
  const album = location.state as IAlbum;
   // Yo queria pasar la cancion como estado, no el album completo, pero se me complico la vida asi que directamente pase el album entero

   const irCancion = (cancionId: number) => {
    navigate(`/album/${album.id}/cancion/${cancionId}`, { state: album });
  };
    
  return (

    <main className='min-h-screen  w-full bg-black flex-col '>
      ...
      <div  className=' h-[300px] overflow-y-auto pr-4 scrollbar-thin scrollbar-thumb-yellow-400 scrollbar-thumb-rounded-md scrollbar-track-transparent mb-10'>
        {album.canciones?.map((cancion) => (
          // Abajo pasamos como key y parametro de la funcion navigate el id de cada elemento cancion
          <div key={cancion.id}  onClick={()=> irCancion(cancion.id)} className='bg-black/50 rounded-[10px] w-[100%] h-10 cursor-pointer text-white justify-between flex border border-white items-center mt-5 hover:bg-[#ad2b3a] transition duration-500 '>
