...
const TiendaProducto = () => {
  ...
  const [imagenMostrada, setImagenMostrada] = useState(dato.productoImagen)
  
  const esVinilo = (dato as IVinilos).tipo !== undefined;

  const imagenClase = esVinilo
    ? "h-[55vh] w-[70vh]"
    : "h-[75vh] w-[75vh]";
  ...
  return (
    ...
    {dato.listaImagen.map((img) => (
      <img
        ...
        src={img.imagen}
        alt={img.imagen}
        onClick={() => setImagenMostrada(img.imagen)}
      />
    ))}
  </div>

        <div className='flex'>
        <img className={imagenClase} src={imagenMostrada} alt={imagenMostrada} />
        </div>
        ...
        <div className='flex flex-col  text-white  max-w-[400px] min-w-[400px] break-words  '>
         ...
         <Link to={"/tienda/producto/compra"} className='mt-4' ><button className='border border-yellow-400 w-[50%] h-10 text-yellow-400 hover:text-black hover:bg-yellow-400 transition duration-300'><p className='font-bold'>COMPRAR</p></button></Link>
